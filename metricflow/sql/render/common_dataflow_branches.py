...
def find_common_branches(node: DataflowPlanNode) -> Sequence[DataflowPlanNode]:
    """Starting from the given node, find the common branches that exist in the associated DAG.
    ...
    return tuple(node.accept(common_branches_visitor).keys())
...
class _FindCommonBranchesVisitor(DataflowPlanNodeVisitor[Dict[DataflowPlanNode, None]]):
    """Given the nodes that are known to be in the DAG multiple times, find the common branches.

    While a set can be used for the output type, a dict is used instead for reproducibility.
    """
